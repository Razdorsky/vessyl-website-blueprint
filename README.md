...
- Page transitions retain native document navigation and history. Ready hero photographs crossfade over an opaque outgoing image while the live incoming photograph settles from 110% to 100% over 800ms inside a stationary, full-width hero capture; text enters over 400ms after a 240ms delay, and header colors blend over 750ms. Warm only intended destinations (HTML and responsive hero images), with no full-site startup loader. Dome, history traversal, unready images and unsupported browsers use the simpler dissolve; reduced motion disables transitions.
